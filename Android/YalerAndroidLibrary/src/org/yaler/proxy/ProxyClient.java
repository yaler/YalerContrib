...
public final class ProxyClient {
	...
	public static final Socket connectProxy(Proxy proxy, String host, int port, int timeout) throws IOException, MalformedHttpResponseException {
		...
		Socket s = null;
		final String basic = "Basic"; //$NON-NLS-1$
		final String digest = "Digest"; //$NON-NLS-1$
		final String ntlm = "NTLM"; //$NON-NLS-1$
		...
		do {
			...
			if (scheme != null) {
				// select AuthScheme
				Header authHeader = new BasicHeader("Proxy-Authenticate", challenge); //$NON-NLS-1$
				try {
				...
				if (proxy instanceof AuthenticatedProxy) {
					...
					PasswordAuthentication auth = Authenticator.requestPasswordAuthentication(
							...
							isAddress.getPort(),
							"http", "", scheme.getSchemeName()); //$NON-NLS-1$ //$NON-NLS-2$
					if (auth == null) {
						throw new ProxyAuthenticationException("credentials could no be fetched from Authenticator", proxy); //$NON-NLS-1$
					}
					cred = new UsernamePasswordCredentials(auth.getUserName(), new String(auth.getPassword()));
				}
				if (cred == null) {
					throw new ProxyAuthenticationException("credentials could not be fetched", proxy); //$NON-NLS-1$
				}
				// authenticate
				try {
					BasicHttpRequest request = new BasicHttpRequest("CONNECT", host+':'+port); //$NON-NLS-1$
					request.addHeader("Proxy-Authenticate", challenge); //$NON-NLS-1$
			...
			if (statusCode == HttpHelper.HTTP_OK) {
				...
				String responseAuthChallenge = headers.get("Proxy-Authenticate"); //$NON-NLS-1$
				if (responseAuthChallenge != null) {
					...
					if (ntlm.equalsIgnoreCase(responseAuthType)) {
						...
						throw new ProxyAuthenticationException("unknown scheme " + responseAuthType, proxy); //$NON-NLS-1$
					}
				} else {
					throw new ProxyAuthenticationException("no challenge provided", proxy); //$NON-NLS-1$
				}
			} else {
				throw new ProxyAuthenticationException("unmanaged http status code " + statusCode, proxy); //$NON-NLS-1$
			}
			if (authorizing) {
				try {
					s.close();
