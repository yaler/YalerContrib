...
YalerESP8266WiFiServer::YalerESP8266WiFiServer(const char *host, uint16_t port, const char *id)
{
  _host = host;
  _port = port;
  _id = id;
...
void YalerESP8266WiFiServer::begin()
{
  // skip
}

WiFiClient YalerESP8266WiFiServer::available()
{
  WiFiClient client;
  boolean acceptable;
...
size_t YalerESP8266WiFiServer::write(uint8_t b) 
{
  return write(&b, 1);
}

size_t YalerESP8266WiFiServer::write(const uint8_t *buffer, size_t size) 
{
  size_t n = 0;
  // TODO
  return n;
